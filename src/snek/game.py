...
class Game:
    def __init__(self, width: int = None, height: int = None, config: GameConfig = None, rng: random.Random = None) -> None:
        ...
        self.world_path = WorldPath(levels_per_world=self.config.levels_per_unicode_phase)
    ...
    def check_level_up(self) -> None:
        """Check if player should level up and update color."""
        if GameRules.should_level_up(self.symbols_consumed, self.level, self.config.symbols_per_level):
            self.level += 1
            # Choose a random color that's different from current
            available_colors = [c for c in self.config.level_colors if c != self.current_color]
